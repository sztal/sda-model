...
```{r sda_skewness, fig.width = 8, fig.asp = .618}
gdf <- sda %>%
    group_by(space, N, m_f, alpha_f) %>%
    summarize(
        stat = mean(deg_skew),
        lower = simple_boot(deg_skew, min),
        upper = simple_boot(deg_skew, max),
        n = n()
    ) %>%
    ungroup
...
gdf %>%
    ...
    facet_grid(space ~ m_f) +
    ylab("Degree distribution skewness")
```
...
gdf <- sda %>%
    ...
    summarize(
        stat = mean(deg_kurt),
        lower = simple_boot(deg_kurt, min),
        upper = simple_boot(deg_kurt, max),
        n = n()
    ) %>%
    ungroup
...
gdf %>%
    make_line_plot(LEGEND_TL, hline = 0) +
    facet_grid(space ~ m_f) +
    ylab("Degree distribution kurtosis")
```
